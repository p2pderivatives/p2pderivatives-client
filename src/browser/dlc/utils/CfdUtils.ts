...
function utxoToUtxoJson(utxo: Utxo): cfdjs.UtxoJsonData {
  return {
    txid: utxo.txid,
    amount: BigInt(utxo.amount.GetSatoshiAmount()),
    vout: utxo.vout,
  }
}
...
export function selectUtxosForAmount(amount: Amount, utxos: Utxo[]): Utxo[] {
  ...
  const reqJson: cfdjs.SelectUtxosRequest = {
    utxos: utxosData,
    targetAmount: BigInt(amount.GetSatoshiAmount()),
  }
