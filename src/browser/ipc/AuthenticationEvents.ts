...
export class AuthenticationEvents extends IPCEventsBase {
    ...
    return [
      {
        ...
        callback: (data): Promise<GeneralAnswer> =>
          this.loginCallback(data as LoginCall),
      },
      {
        tag: LOGOUT,
      ...
      },
      {
        tag: CHANGE_PASSWORD,
        callback: (data): Promise<GeneralAnswer> =>
          this.changePasswordCallback(data as ChangePasswordCall),
      },
      {
        tag: GET_USER,
  ...
  private async loginCallback(request: LoginCall): Promise<GeneralAnswer> {
    try {
      await this._client
