...
export class DlcEvents extends IPCEventsBase {
  ...
  protected taggedCallbacks(): TaggedCallback[] {
    return [
      {
        tag: DLC_EVENT,
        callback: (data): Promise<DlcAnswer> => this.dlcEventsCallback(data),
      },
      {
        tag: OFFER_CONTRACT,
        callback: (data): Promise<DlcAnswer> =>
          this.offerContractCallback(data),
      },
      {
        tag: GET_CONTRACTS,
  ...
  private async dlcEventsCallback(data: unknown): Promise<DlcAnswer> {
  ...
  private async offerContractCallback(data: unknown): Promise<DlcAnswer> {
