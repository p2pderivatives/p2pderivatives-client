...
export const balanceSuccess = (balance: number) => action(BitcoinActionTypes.BALANCE_SUCCESS, balance)
export const balanceError = (error: string) => action(BitcoinActionTypes.BALANCE_ERROR, error)
export const configRequest = () => action(BitcoinActionTypes.CONFIG_REQUEST)
export const configRetrieved = (config: BitcoinDConfig) => action(BitcoinActionTypes.CONFIG_RETRIEVED, config)
